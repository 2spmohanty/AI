...
from langchained_triage import run_lang_chained_v2
from langchain_output_eval import evaluate_agent_output

def execute_diagnostic_pipeline(sample_name: str, log_payload: str, agent_type="v1",lang_chained=False):
    ...
    if lang_chained:
        print("=" * 80)
        print("Running diagnostic pipeline with Lang chained V2 ReAct Agentic Pipeline")
        diagnosis = run_lang_chained_v2(log_payload,5)
        ...
        verdict = evaluate_agent_output(sample_name, diagnosis)
...
if __name__ == "__main__":
    ...
    log_data : str = None
    with open(dynamo_access_log_file, 'r', encoding='utf-8') as f:
        log_data = f.read()
    ...
    execute_diagnostic_pipeline(f"REACT-ANALYSIS-{current_time}", log_data,lang_chained=True)
